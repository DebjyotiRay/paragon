...
export class SettingsView extends LitElement {
    ...
    async loadInitialData() {
        ...
        try {
            ...
            const [settings, presets, apiKey, contentProtection, userState, provider] = await Promise.all([
                ...
                ipcRenderer.invoke('get-ai-provider')
            ]);
            
        this.settings = settings;
        // Filter out school and recruiting presets
        this.presets = (presets || []).filter(p => p.id !== 'school' && p.id !== 'recruiting');
        this.apiKey = apiKey;
            ...
            // Set the provider select dropdown to the current provider
            this.updateComplete.then(() => {
                const providerSelect = this.shadowRoot?.getElementById('provider-select');
    ...
    handleProviderChange(event) {
        ...
        console.log(`[SettingsView] Provider changed to: ${provider}`);
    ...
    async handleSaveApiKey() {
        ...
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            try {
                const result = await ipcRenderer.invoke('api-key-validated', {
                    apiKey: newApiKey,
                    provider: provider
                });
                
                if (result.success) {
                    console.log(`API Key saved successfully with provider: ${provider}`);
                ...
                console.error('Error saving API key and provider:', e);
            }
        }
    }

    async handleSaveAwsCredentials() {
        const accessKeyInput = this.shadowRoot.getElementById('aws-access-key-input');
        const secretKeyInput = this.shadowRoot.getElementById('aws-secret-key-input');
        const sessionTokenInput = this.shadowRoot.getElementById('aws-session-token-input');
        const providerSelect = this.shadowRoot.getElementById('provider-select');
        
        if (!accessKeyInput || !accessKeyInput.value || !secretKeyInput || !secretKeyInput.value) {
            console.error('AWS Access Key and Secret Key are required');
            return;
        }

        const accessKey = accessKeyInput.value;
        const secretKey = secretKeyInput.value;
        const sessionToken = sessionTokenInput?.value || null;
        const provider = 'bedrock';
        
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            try {
                // First store AWS credentials in environment
                await ipcRenderer.invoke('set-aws-credentials', {
                    accessKey, 
                    secretKey,
                    sessionToken,
                    region: 'us-east-1'
                ...
                // Then save the access key and provider using the existing API
                const result = await ipcRenderer.invoke('api-key-validated', {
                    apiKey: accessKey, // Store access key as the API key
                    provider: provider
                });
                
                if (result.success) {
                    console.log(`AWS credentials saved successfully with provider: ${provider}`);
                    this.awsAccessKey = accessKey;
                    this.awsSecretKey = secretKey;
                    this.awsSessionToken = sessionToken;
                    this.requestUpdate();
                } else {
                    console.error('Failed to save AWS credentials:', result.error);
                }
            } catch(e) {
                console.error('Error saving AWS credentials:', e);
            }
        }
    }
        ...
        return html`
            <div class="settings-container">
                ...
                <div class="api-key-section">
                    <!-- Provider selection -->
                    <select id="provider-select" class="settings-button full-width" @change=${this.handleProviderChange} ?disabled=${loggedIn} style="margin-bottom: 4px;">
                        <option value="openai">OpenAI</option>
                        <option value="gemini">Gemini</option>
                        <option value="bedrock">AWS Bedrock</option>
                    </select>

                    <!-- Standard API Key input (for OpenAI/Gemini) -->
                    <div id="standard-key-input" class="${this.currentProvider !== 'bedrock' ? '' : 'hidden'}">
                        <input 
                            type="password" 
                            id="api-key-input"
                    ...
                    <!-- AWS Bedrock credentials inputs -->
                    <div id="aws-credentials-inputs" class="${this.currentProvider === 'bedrock' ? '' : 'hidden'}">
                        <input 
                            type="password" 
                            id="aws-access-key-input"
                            placeholder="AWS Access Key ID" 
                            .value=${this.awsAccessKey || ''}
                            ?disabled=${loggedIn}
                        ...
                        <input 
                            ...
                            .value=${this.awsSessionToken || ''}
                            ?disabled=${loggedIn}
                            style="margin-bottom: 4px;"
                        >
                        <button class="settings-button full-width" @click=${this.handleSaveAwsCredentials} ?disabled=${loggedIn}>
